...
class MultipleSelect extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        const { selctOptions } = nextProps;
        this.setState({
            selctOptionState: selctOptions
        });
    };
    ...
    conditonChange(e, index, event) {
        event.stopPropagation();
        event.nativeEvent.stopImmediatePropagation();
        const { onSelect, selctOptions } = this.props;
        if (1 === index) return;
        this.setState({
            activeIndex: index
        });
                                            ...
                                            ["Select All", " - ", "Clear"].map((item, index) => {
                                                return (
                                                    <div
                                                        ...
                                                        className={
                                                            index === activeIndex ||
                                                            (selctOptions.length === defaultValue.length
                                                                &&
