    ...
    for (let i = 0; i < configs.length; i++) {
        ...
        for (let key in keyObject) {
            if (data.hasOwnProperty(key)) {
                prepareData[title][keyObject[key]] = data[key];
            }
            if (key === "logical.sensorStatus") {
                prepareData[title][keyObject[key]] = data["properties"] && data["properties"]["sensorstatus"];
            }
        }
    }
...
function loopSearchDeviceData(dataObject, word, copyObject) {
    let searchWord = word;
    for (let i = 0; i < dataObject.length; i++) {
        let object = dataObject[i];
        let key = object.displayname;
        ...
        //         // copyObject[key] = {};
        //     }
        //     loopSearchDeviceData(value, searchWord, copyObject, object);
        // } else
        if (key.toLowerCase().indexOf(searchWord) !== -1 || value.toLowerCase().indexOf(searchWord) !== -1) {
