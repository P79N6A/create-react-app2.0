...
import { activitionUser, isValidToken } from "api/security";
const modulesName = "Activation - User";

function* activitionUserData({ postData }) {
    try {
        let result = yield call(activitionUser, postData);
...
function* activitionUserDataSaga() {
    yield takeLatest(actionTypes.SECURITY_ACTIVITION_USER_REQUEST, activitionUserData);
}

function* validationTokenSaga() {
    yield takeLatest(actionTypes.SECURITY_ACTIVITION_ISVAILTOEKN, validationToken);
}
