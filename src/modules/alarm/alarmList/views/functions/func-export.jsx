...
import { REDUCER_NAME as reducerName, alarmExportConfig } from "modules/alarm/alarmList/funcs/constants";
import { exportAlarmDataRequest, exportAlarmDataSuccess } from "modules/alarm/alarmList/funcs/actions";
...
export class ExportComps extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (_.isEqual(nextProps, this.props) || !exportInfo || source !== "alarmList") {
            return;
        }
    ...
    callExport(nextProps) {
        ...
        if (checkCurrentWidget && checkExportTime) {
            ...
            let columninfos = alarmExportConfig;
            let date = dateRange && dateRange.length === 1 ? [getTimeString(dateRange[0])] : dateRange;
            let pageLimit = nextProps[identify].pageLimit;
            let itemsData = nextProps[identify].itemsData.concat(_.cloneDeep(nextProps[identify].filterDataList));
            let arr = handleDataForRequest(itemsData);
            let predicate = {
                operator: "AND",
                predicates: arr
            };
            this.props.exportAlarmData(identify, predicate, columninfos, pageLimit, date, this.props.applicationid);
...
const mapDispatchToProps = dispatch => {
    return {
        exportAlarmData: (identify, itemsData, columninfos, pageLimit, dateRange, applicationid) => {
            dispatch(exportAlarmDataRequest(identify, itemsData, columninfos, pageLimit, dateRange, applicationid));
        },
        clearExportData: (identify, data) => {
            dispatch(exportAlarmDataSuccess(identify, data));
        },
        callReminder: val => {
            dispatch(msg.info(val, "Alarm - Export"));
        }
    };
};
