...
import { addRule, getTopologyModelInfoSuccess, updateNotification } from "../funcs/actions";
...
const EVENT_TYPE = "Device/Reading";
...
class AddRule extends React.Component {
    ...
    inital(props) {
        ...
        const { ruleSchema, actionSchema } = props;
    ...
    getConfigData(ruleSchema, actionSchema) {
        ...
        for (let key in ruleSchema) {
            if (key !== "name" && key !== "description") {
                if (key === "time_range") {
                    config["configvalue"]["time_from"] = ruleSchema[key]["time_from"].default;
                    config["configvalue"]["time_to"] = ruleSchema[key]["time_to"].default;
                    continue;
                } else if (key === "day") {
                    const value = ruleSchema[key].default;
                    config["configvalue"][key] = value ? (value instanceof Array ? value : value.split(",")) : value;
                } else if (key === "device") {
                    config["configvalue"][key] = {};
                } else if (key === "reason") {
                    config["configvalue"][key] = "";
                } else if (key === "suppressperiodmin" || key === "timewindowsec") {
                    config["configvalue"][key] = "0";
                } else {
                    config["configvalue"][key] = ruleSchema[key].default;
                }
            } else {
                config[key] = "";
            }
    ...
    confirmSave = () => {
        const { config, action, showTime, dashboardList } = this.state;
        const { userid, sitename, identify, pageLimit, modulename, actionContent } = this.props;
        ...
        for (let key in configvalue) {
            if (configvalue[key] instanceof Array) {
                configvalue[key] = configvalue[key].join();
            }
        }
        ...
        if (!showTime) {
            delete configvalue["time_from"];
            delete configvalue["time_to"];
        }
        configvalue["timezone"] = moment().format("ZZ");
        configvalue["eventtype"] = EVENT_TYPE;
    ...
    getConditionConfig(data) {
        let { config } = this.state;
        const mode = config["configvalue"]["device"] && Object.keys(config["configvalue"]["device"])[0];
        if (config["configvalue"]["device"] && config["configvalue"]["device"][mode]) {
            config["configvalue"]["condition"] = data;
        } else {
    ...
    getDeviceConfig(data) {
        let { config } = this.state;
        if (!_.isEqual(config["configvalue"]["device"], data)) {
            config["configvalue"]["condition"] = {};
        }
    ...
    generateCondition() {
        ...
        const generateConfig = config["configvalue"] && config["configvalue"]["condition"];
    ...
    generateGeneral() {
        ...
        // <List className="rule-group">{this.generateRuleSchema()}</List>;
        return <RuleInfoProcess
