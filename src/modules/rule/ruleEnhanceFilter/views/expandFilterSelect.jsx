...
class ExpandFilterSelect extends React.Component {
    ...
    componentWillMount() {
        if (this.props.filter.filterConfigType !== "DeviceType" || !this.props.ruleTypeDatas) {
            return;
        }
        let filter = this.state.filter;
        let ruleTypeDatas = this.props.ruleTypeDatas;
        let ruleTypes = [];
        for (let i = 0; i < ruleTypeDatas.length; i++) {
            ruleTypes.push(ruleTypeDatas[i]["devicetype.displayName"]);
        }
        filter.values = ruleTypes;
        this.setState({
            filter: filter
        });
    }
    ...
    componentWillReceiveProps(nextProps) {
        if (this.props.filter.filterConfigType === "DeviceType" && nextProps.ruleTypeDatas) {
            let filter = this.state.filter;
            let ruleTypeDatas = nextProps.ruleTypeDatas;
            let ruleTypes = [];
            for (let i = 0; i < ruleTypeDatas.length; i++) {
                ruleTypes.push(ruleTypeDatas[i]["devicetype.displayName"]);
            }
