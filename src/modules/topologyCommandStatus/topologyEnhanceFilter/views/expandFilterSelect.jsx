...
class ExpandFilterSelect extends React.Component {
    ...
    componentWillMount() {
        if (this.props.filter.filterConfigType !== "Device Type" || !this.props.topoTypeDatas) {
            return;
        }
        if (this.state.currentSelect.length) {
            return;
        }
        ...
        for (let i = 0; i < topoTypeDatas.length; i++) {
            topoTypes.push(topoTypeDatas[i]["devicemodel.displayName"]);
        }
    ...
    componentWillReceiveProps(nextProps) {
        ...
        //     return;
        // }
        if (this.props.filter.filterConfigType === "Device Type" && nextProps.topoTypeDatas) {
            let filter = this.state.filter;
    ...
    render() {
        ...
        return (
            <FormControl className={classes.formControl} style={{ width: "227px", marginLeft: 0 }}>
                <InputLabel
                    ...
                    {filter.values &&
                        filter.values.map((item, index) => (
                            <MenuItem
                                ...
                                value={item}
                                title={item}
                                style={{ width: "300px", manWidth: "300px" }}
                            >
                                <Checkbox
