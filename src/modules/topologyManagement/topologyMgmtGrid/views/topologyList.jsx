...
class TopologyList extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (
            ...
            !columnConfig ||
            !selected ||
            (!nextProps.checkedTab && nextProps.searchType !== "device") ||
            (nextProps.checkedTab && nextProps.searchType !== "devicetype")
        ) {
            return;
        }
        this.prepareDataAndColumns(
            arrayData,
            columnConfig,
            nextProps.pagination,
            nextProps.checkedTab,
    ...
    prepareDataAndColumns(arrayData, columnConfig, pagination, checkedTab, sysconfigDevicetypes) {
        let defaultColumns = prepareDefaultColumns(columnConfig, checkedTab);
                ...
                for (let j = 0; j < sysDevictypes.length; j++) {
                    ...
                    if (arrayData[i]["devicemodel.iotTopologyId"] === sysDevicetypeId) {
                        arrayData[i]["description"] =
                            sysDevictypes[j]["configvals"] &&
                            sysDevictypes[j]["configvals"][0] &&
                            sysDevictypes[j]["configvals"][0]["configvaldesc"];
                        let value = JSON.parse(
                            sysDevictypes[j]["configvals"] &&
                                sysDevictypes[j]["configvals"][0] &&
                                sysDevictypes[j]["configvals"][0]["configval"]
                        );
        ...
        let datas = prepareRenderDatas(arrayData, defaultColumns, checkedTab);
    ...
    render() {
        ...
        const { refreshTopologySuccess, checkedTab } = this.props;
        return (
            <div className="topology-list">
                <div className="topology-list-wrap">
                    {selected.length > 0 && multipleSelect ? (
                        <SelectedRowInfo identify={this.props.identify} checkedTab={checkedTab} selected={selected} />
                    ) : null}
...
const mapStateToProps = (state, ownProps) => {
    ...
    return {
        ...
        searchType: filterProps(state, identify, topoReducerName, "searchType")
    };
};
