...
function prepareDefaultColumns(defaultColumnsConfig, checkedTab) {
    let totalColumnsConfig = [];
    if (!checkedTab) {
        totalColumnsConfig = TopologyConfig.tableColumnsConfig;
    } else {
        totalColumnsConfig = TopologyConfig.tableTypeColumnsConfig;
    }
...
function prepareRenderDatas(arrayData, defaultColumns, checkedTab) {
    let opts = [];
    for (let i = 0; i < arrayData.length; i++) {
        ...
        let key = !checkedTab
            ? arrayData[i]["physical.iotTopologyId"] +
              ...
              "_" +
              arrayData[i]["physical.displayName"] +
              "_" +
              arrayData[i]["address.iotTopologyId"] +
              "_" +
              arrayData[i]["address.name"] +
              "_" +
              arrayData[i]["devicemodel.iotTopologyId"]
            : arrayData[i]["devicemodel.iotTopologyId"];
        opt = Object.assign({}, opt, {
            key: key
        });
