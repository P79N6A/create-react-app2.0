            ...
            _.forEach(formatBasicTypesList, function(typeList) {
                for (let innerkey in typeList) {
                    if (basictype[key].type.toString() === innerkey.toString()) {
                        ...
                        obj["basictypeunit"] = basictype[key].units;
                        obj["basictypeRW"] = basictype[key].operations;
...
function formatDevicePropertyValueForEdit(devicetypes, devicedata) {
    let deviceTypeId = devicedata["devicemodel.iotTopologyId"] || "";
    let deviceProperties = devicedata.properties;
    let propertyValues = { readWrite: [], readOnly: [], execute: [] };
    let propertyInitValue = {};
    _.forEach(devicetypes, devicetype => {
        let sysconfigDevicetypeId = devicetype.configname;
        if (sysconfigDevicetypeId === deviceTypeId) {
            let currentDeviceType =
                jp.query(devicetype, configValuePath) && JSON.parse(jp.query(devicetype, configValuePath)[0]);
            let currentBasictypes = jp.query(currentDeviceType, "$..['basicTypeInstances']")[0];
            _.forEach(currentBasictypes, basictype => {
                for (let key in basictype) {
                    if (deviceProperties.hasOwnProperty(key)) {
                        basictype[key].displayname = key;
                        basictype[key].value = deviceProperties[key];
                        propertyInitValue[key] = deviceProperties[key];
                        if (basictype[key].operations === "RW") {
                            propertyValues.readWrite.push(basictype[key]);
                        } else if (basictype[key].operations === "R") {
                            propertyValues.readOnly.push(basictype[key]);
                        } else if (basictype[key].operations === "E") {
                            propertyValues.execute.push(basictype[key]);
                        }
                    }
                }
            });
    ...
    return { propertyValues, propertyInitValue };
}
