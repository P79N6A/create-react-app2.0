...
export default class deviceMapL extends Component {
    constructor(props) {
        super(props);
        this.map = {};
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        const { dataSource, icon, iconColor } = this.props;
        this.addMarker(dataSource, icon, iconColor);
    };

    addMarker = (dataSource, icon, iconColor) => {
        let markerGroup = [];
        if (dataSource && dataSource.length) {
            this.removeMarker();
            dataSource.forEach((item) => {
                markerGroup.push(
                    L.marker(map_.handleCoordinate(item.center),
                        map_.addMarker(
                            icon,
