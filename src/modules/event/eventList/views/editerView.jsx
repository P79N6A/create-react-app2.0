...
import { editorControlProps, getItemsRequest, requestParameters, saveItemsData } from "../funcs/actions";
// import { reducerName as topoReducerName } from "modules/event/eventList";
...
import EventFilter from "./filter2";
...
class EditerView extends Component {
    ...
    handleInputSelectChanged(propertyType, event) {
        ...
        if (propertyType === "pageLimit") {
            this.getItemsSearch(0, event.target.value, this.state.defaultFilterData);
        }
    ...
    getData = () => {
        // console.log("save editData to ccms:", this.state);
        this.props.editorControlProps(this.state);
    ...
    addFilterItem = () => {
        ...
        // this.state.defaultFilterData.push(fieldItems);
        let defaultFilterData = _.cloneDeep(this.state.defaultFilterData);
        defaultFilterData.push(fieldItems);
        this.setState(
            {
    ...
    removeFilterDataFunc = defaultFilterData => {
        if (!defaultFilterData.length) {
    ...
    isLock = () => {
        ...
        let lock = false;
        let dataList = this.state.defaultFilterData;
                    ...
                    <ListItem style={{ paddingLeft: 0, paddingRight: 0 }}>
                        <FormControl className="filterConfig">
                            <EventFilter
                                ...
                                removeFilterData={this.removeFilterDataFunc.bind(this)}
                                defaultFilterData={this.state.defaultFilterData}
